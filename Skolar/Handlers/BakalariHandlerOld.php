...
class BakalariHandler2 implements \Skolar\Handlers\BaseHandler {
    ...
    /** @var \Skolar\Modules\Bakalari\LoginModule Přihlašování  */
    private $login = null;
    
    /** @var \Skolar\Modules\Bakalari\NavigaceModule Navigace  */
    private $nav = null;
    ...
    public function __construct(\Skolar\Modules\BaseModule $module, $parameters) {
        $this->login = new \Skolar\Modules\Bakalari\LoginModule();
        $this->nav = new \Skolar\Modules\Bakalari\NavigaceModule();
    ...
    private function outputHttp() {
        try {
            //oprav odkaz
            $this->params['url'] = \Skolar\Utils::getFixedUrl($this->params['url']);
            ...
            if($response instanceof \Skolar\Response) {
                return $response;
            }
            ...
            if($module_params->url == false){
                return (new \Skolar\Response())->setError("Neexistující požadavek");
            }
            ...
            return (new \Skolar\Response())->setError("Nemohu se připojit na server");
        } 
    }
     ...
     * 
     * @param \Symfony\Component\DomCrawler\Crawler $response
     * @param \Skolar\Parameters $module_params
     * @return \Symfony\Component\DomCrawler\Crawler
     */
    private function fillParameters($response, $module_params) {
        
        try {
            $arguments = $response->filterXPath("//form")->form($module_params->optional)->getPhpValues();
